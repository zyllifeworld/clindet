...
if genome_version in ['hg19','b37','hg38','hg38_EBV']:
    ...
    rule CM_call: 
        ...
        shell:
            ...
            -s {params.s} -sa {wildcards.genome_version} -seqType {params.seqtype} -t {threads} -no-flagging
            touch {output.log}
            """
else:
    ...
    rule CM_call: 
        ...
        shell:
            ...
            -tumour-bam {input.Tum} -normal-bam {input.NC} \
            -ignore-file {params.igbed} \
            -s {wildcards.genome_version} -sa {wildcards.genome_version} -seqType {params.seqtype} -t {threads} -no-flagging
            touch {output.log}
            """
