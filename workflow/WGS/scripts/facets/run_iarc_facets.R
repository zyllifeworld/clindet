...
ugcpct_rds = commandArgs(TRUE)[8] # 20 
# print(ugcpct_rds)
if (!is.na(ugcpct_rds)){
  load(ugcpct_rds)
  ugcpct = gcpctdb
...
# cval_preproc :
if (genome %in% c("hg19", "hg38", "hg18", "mm9", "mm10")){
  xx = preProcSample(rcmat, gbuild = genome, ndepth = ndepth_param, snp.nbhd = cur_params[1], cval = cur_params[2])
} else {
  genome_new = 'udef'
  xx = preProcSample(rcmat, gbuild = genome_new,ugcpct=ugcpct, ndepth = ndepth_param, snp.nbhd = cur_params[1], cval = cur_params[2])
}
