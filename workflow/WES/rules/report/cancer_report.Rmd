...
if(is.null(sv_available) | !sv_available) {
  no_sv_found <- FALSE
}
...
qc_summary_all <- dplyr::tribble(
  ...
  1, "Sample Name", tumor_name, "Sample Name of this run",
...
qc_summary_all %>%
  ...
  gt::opt_row_striping() %>%
  gt::tab_options(table.align = "left",column_labels.hidden = TRUE,table.width = gt::pct(40)) 
...
## **Somatic Mutation Profiles** {#snv-summary}
...
```{r circos_default1, out.width="50%"}
...
```{r purple_rainfall, out.width="50%"}
knitr::include_graphics(file.path(purple_dir,'plot', paste0(params$tumor_name, ".somatic.rainfall.png")))
```
...
```{r circos_default2, out.width="50%"}
...
```{r plot_bnd_sf_df_tot_lines, fig.width=12, fig.height=12, out.width="90%",eval=report_sv}
...
purple_qc_summary %>%
  ...
  gt::tab_style(
    ...
    locations = gt::cells_body(
      columns = value,
      rows = grepl("FAIL", value) | 
             value == "FALSE" | 
             value == "MALE_KLINEFELTER"
    )
  ) %>%
  gt::tab_style(
  ...
  gt::tab_options(
    table.width = gt::pct(30),  # table width
